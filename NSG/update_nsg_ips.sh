...
# az network nsg rule create 限制说明：
# 1. IP 和网段数量：单条安全规则中，源地址或目的地址前缀的最大数量限制为 4000 个。
#    - 注意：如果 IP 个数过多，可能会超出操作系统命令行的最大字符限制，导致执行报错。
#    - 大量 IP 的场景更推荐使用应用安全组 (ASG) 进行统一管理。
# 2. 优先级 (priority)：必须是 100 到 4096 之间的整数，且同方向（Inbound/Outbound）下的规则优先级必须唯一。
# 3. 端口范围：合法值为 0-65535，可以使用单个端口、连续端口（如 80-100）或全部端口（'*'）。
